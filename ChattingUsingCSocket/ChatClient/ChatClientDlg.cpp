...
void CChatClientDlg::OnSend() 
{
	UpdateData(TRUE);
	UserInfo_t uinf;
	wcscpy(uinf.cszUserName,m_sUserName);
	wcscpy(uinf.cszMsg, m_sMsg);

	wchar_t *buff = (wchar_t*)&uinf;

	m_Client.Send(buff,sizeof(UserInfo_t));
...
void CChatClientDlg::AddMsg(UserInfo_t *udata)
{
	CString sDisplay;

	sDisplay.Format(L"%s : %s",udata->cszUserName,udata->cszMsg);
