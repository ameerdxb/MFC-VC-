...
void CChatServerDlg::OnSendInfo() 
{
	...
	wcscpy(uinf.cszUserName,L"Server");
	wcscpy(uinf.cszMsg, m_Msg);

	char *buff=(char*)&uinf;

	m_Server.m_Client.Send(buff,sizeof(UserInfo_t));
...
{
	AfxMessageBox(L"Client Request Connection");
	if(m_Server.Accept(m_Server.m_Client))
		return;

}

void CChatServerDlg::AddMsg( UserInfo_t *udata)
{
	CString sUserName;

	sUserName.Format(L"%s : %s",udata->cszUserName,udata->cszMsg);
